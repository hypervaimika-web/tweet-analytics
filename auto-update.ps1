# Tweet Analytics - Auto Update Script
# Fetches tweets and outputs data.json for manual/automated upload
...
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Fetching tweets from @Strakyo..."
...
if (-not $tweets) {
    Write-Host "ERROR: Failed to fetch tweets"
    exit 1
}

Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Fetched $($tweets.Count) tweets"
...
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Saved to $jsonPath"
Write-Host "Stats: Tweets=$($tweets.Count) Likes=$totalLikes Replies=$totalReplies Retweets=$totalRetweets Engagements=$totalEngagements"

# Output path for automation
Write-Output "DATA_JSON_PATH=$jsonPath"
